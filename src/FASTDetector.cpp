...
bool FASTDetector::detect(const cv::Mat &img, double threshold, std::vector<cv::KeyPoint> &kps)
{
    kps.clear();
    ...
    int windows_size = 7;
    // selectMax(img, windows_size, kps);
    if (kps.size() != 0)
        return true;
    else
        return false;
...
void FASTDetector::selectMax(const cv::Mat &img, int windows_size, std::vector<cv::KeyPoint> &kps)
{
    int u, v;
    // std::vector<size_t>
    for (auto kp : kps)
...
// 计算ORB描述子BRIEF（改进）
void FASTDetector::ORBdesc(const cv::Mat &img, std::vector<cv::KeyPoint> &kps, std::vector<uchar> &descs)
{
